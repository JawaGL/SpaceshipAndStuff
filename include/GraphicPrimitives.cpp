...
void GraphicPrimitives::drawFillConvexPolygoneP2D(std::vector<float *>& x,std::vector<float *>& y,float r,float g, float b,float a ){
    ...
    for (int i=0; i <= x.size()-3; i++) {
        glVertex3f (*x[x.size()-1], *y[y.size()-1],0);
        glVertex3f (*x[i],*y[i],0);
        glVertex3f (*x[i+1],*y[i+1],0);
        /*glVertex3f (*x[x.size()-1], *y[y.size()-1],0);
        glVertex3f (*x[i],*y[i],0);
        glVertex3f (*x[x.size()-2], *y[y.size()-2],0);*/
    }
 /*   glVertex3f (*x[0],*y[0],0);
    glVertex3f (*x[x.size()-2],*y[x.size()-2],0);
    glVertex3f (*x[x.size()-1],*y[x.size()-1],0);
